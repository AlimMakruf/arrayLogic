...
function count(twice) {
    for (let i = 0;i<twice.length;i++) {
        if (typeof twice[i] !== 'number') {
...
function reverse(blackpink) {
    for (let i = 0;i<blackpink.length;i++) {
        if (typeof blackpink[i] !== 'number') {
            return 'Error, tipe data harus number'
        }
    }
    let array = [];
    for (let i = 0; i<blackpink.length;i++) {
        let blackpinkString = blackpink[i].toString();
        // console.log(typeof arrString);
        let reverseArray = ''
        for(let j=blackpinkString.length - 1; j>= 0; j--) {
            reverseArray += blackpinkString[j];
        }
